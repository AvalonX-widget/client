...
class PostDetail extends Component {
    ...
    getData = async () => {
        ...
        const result = await axios.get(`http://localhost:8080/posts/${id}`, { withCredentials: true });
    ...
    like = async() => {
        try {
            const result = await axios.post(`http://localhost:8080/likes/${this.state.id}`, {}, { withCredentials: true });
    ...
    onInputComment = async (e) => {
        ...
        if (e.key === 'Enter') {
            ...
            const result = await Axios.post(`http://localhost:8080/posts/${this.state.id}/comments`, { text: e.target.value }, { withCredentials: true});
            const new_comment = {
    ...
    render() {
        ...
        return (
            <div className={styles.detail} style={{padding:'250px'}}>
