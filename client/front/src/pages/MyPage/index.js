...
class MyPage extends Component {
	...
	fetchUser = async () => {
		const {data: { user }} = (await Axios.get('http://localhost:8080/users/mypage', { withCredentials: true }));
		if (!user) {
	...
	handleSubmit = async (e) => {
		...
		const result = await Axios.post(`http://localhost:8080/users/update`, formData, { withCredentials: true });
		if (profile_updated) {
			this.setState({
	...
	handleDelete = async (e) => {
		...
		if(window.confirm('정말 탈퇴하시겠습니까?')) {
			await Axios.post('http://localhost:8080/users/delete', {}, { withCredentials: true });
